...
class excitation_729_ramp(experiment):
    ...
    def setDACVoltages(self,cxn):
        dac_control = self.parameters.DACcontrol
        num = dac_control.num_steps
        U2startdict = cxn.dac_server.get_multipole_values() 
        U2start = dict(U2startdict)['U2']
        U2end = dac_control.U2target
        U2mid = (U2start - U2end)/2.0
        curr = 1.0
        self.voltages = []
        # ramp down
        while (curr <= num):
        ...
        curr = 1.0
        while (curr <= num):
            val = curr/num *(U2start-U2end) + U2end
            self.voltages.append(val)
            curr = curr + 1.0

        #set Voltages
        cxn.dac_server.setArrayU2(self.voltages)
    ...
    def run(self, cxn, context):
        ...
        if(self.parameters.DACcontrol.enable_ramp):
            self.setDACVoltages(cxn)
        if self.use_camera:
