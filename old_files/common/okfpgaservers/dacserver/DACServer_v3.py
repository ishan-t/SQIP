...
class DACServer( LabradServer ):
    ...
    def initServer( self ):
        ...
        self.queue = Queue()
        self.queue.reset()
        self.pulser.switch_manual('rst',False)
        self.pulser.switch_manual('adv',False)
        self.pulser.switch_manual('adv',True)
        self.pulser.switch_manual('rst',True)
        self.pulser.switch_auto('adv')
        self.pulser.switch_auto('rst')
        self.current = {}
        self.listeners = set() 
    ...
    def setPreviousVoltages( self ):
        ...
        try:
            ...
            self.multipoleMatrix = {k: {j: .1 for j in hc.default_multipoles} for k in hc.elec_dict.keys()} # if no previous Cfile was set, set all entries to zero.
            self.numCols = 1
        try: ms = yield self.registry.get('default_multipoleset')                    
        except: ms = [(k, 0) for k in hc.default_multipoles] # if no previous multipole values have been recorded, set them to zero. 
    ...
    def setMultipoleControlFile(self, c, CfilePath):                
        ...
        if self.numCols == 1: 
            self.multipoleMatrix = {elec: {mult: data[int(elec) + index*hc.numElectrodes - 1] for index, mult in enumerate(hc.default_multipoles)} for elec in hc.elec_dict.keys()}
    ...
    def interpolateMultipoleMatrix( self, data ):
        ...
        splineFit = {elec: {mult: UniSpline(range(self.numCols) , data[int(elec) + index*hc.numElectrodes - 1], s = 0 ) for index, mult in enumerate(hc.default_multipoles)} for elec in hc.elec_dict.keys()}
        self.multipoleMatrix = {elec: {mult: splineFit[elec][mult](partition) for index, mult in enumerate(hc.default_multipoles)} for elec in hc.elec_dict.keys()}
        positionFit = interpolate.interp1d(range(self.numCols) , data[numElectrodes * len(hc.default_multipoles)], 'linear')
    ...
    def setMultipoleValues(self, c, ms):
        ...
        """
        self.default_multipoleset = {}
        for (k,v) in ms:
            self.default_multipoleset[k] = v
        yield self.setVoltages(c, self.currentPosition)

        yield self.registry.cd(self.registryPath)
        yield self.registry.set('default_multipoleset', ms)
    ...
    def getMultipoleValues(self, c):
        ...
        """
        return self.default_multipoleset.items()
    ...
    def setVoltages(self, c, newPosition = currentPosition):
        ...
        for e in hc.elec_dict.keys():        
            av = 0
            for m in hc.default_multipoles:                 
                if self.numCols == 1: av += self.multipoleMatrix[e][m] * self.default_multipoleset[m] # numpy.0-dimnl_array[0] throws error
                else: av += self.multipoleMatrix[e][m][n] * self.default_multipoleset[m] # numpy.array[0] = scalar
            newVoltageSet.append( (e, av) )
        # if changing DAC FPGA voltage set, write sma voltages. 
        #if advance or reset:
        #    for s in hc.sma_dict.keys():
    ...
    def setNextU2(self, c, U2):
        self.default_multipoleset['U2'] = U2
        self.queue.advance()
        yield self.setVoltages(c)
...
Be sure to specify multipole names in DacCanfiguration.py, e.g., default_multipoles = ['Ex', Ey', 'Ez', 'U2'].
