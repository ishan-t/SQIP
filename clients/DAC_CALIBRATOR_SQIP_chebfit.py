...
class DAC_CALIBRATOR(QDACCalibrator):
    ...
    def calib(self):
        ...
        if self.checkreverse.isChecked() == False:
            self.digVoltages = range(2**8+2000, 2**16-2**8, stepsize)
        ...
        if int(self.channelToCalib) < 10:
            channel = '0' + str(int(self.channelToCalib))
        else:
            channel = str(int(self.channelToCalib))
            
        self.dacserver.set_individual_digital_voltages([(channel, int(0))])
        #time.sleep(1)
        for dv in self.digVoltages: # iterate over digital voltages

            self.dacserver.set_individual_digital_voltages([(channel, int(dv))]) 
        ...
        if self.checksave.isChecked() == True:         
            ...
            self.r.cd( ( ['Servers', 'SQIPDAC Server', 'Calibrations', self.channelToCalib], True ) )
            self.r.set( ( 'c0', fit[0] ) )
            self.r.set( ( 'c1', fit[1] ) )
            self.r.set( ( 'c2', fit[2] ) )
            self.r.set( ( 'c3', fit[3] ) )
